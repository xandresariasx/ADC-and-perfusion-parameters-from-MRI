function [P, J] = MCC_regionGrowing(cIM, initPos, thresVal, maxDist, tfMean, tfFillHoles, tfSimplify)
% MCC_REGIONGROWING Region growing algorithm for 2D/3D grayscale images
%
% Syntax:
%   P = MCC_regionGrowing();
%   P = MCC_regionGrowing(cIM);
%   P = MCC_regionGrowing(cIM, initPos)
%   P = MCC_regionGrowing(..., thresVal, maxDist, tfMean, tfFillHoles, tfSimpl)
%   [P, J] = MCC_regionGrowing(...);
%
% Inputs:
...
%   poly = MCC_regionGrowing(cIM, [], 300); % click somewhere inside the lungs
%   plot(poly(:,1), poly(:,2), 'LineWidth', 2)
%   
%   % 3D Example
%   load mri
%   poly = MCC_regionGrowing(squeeze(D), [66,55,13], 60, Inf, [], true, false);
...
disp(['MCC_regionGrowing Opening: Initial position (' num2str(initPos(1))...
...
disp(['MCC_regionGrowing Ending: Found ' num2str(length(find(J)))...
