function  imshow3DV3( Img, disprange,Title,Locations,S )
...
% -=< Slice slider callback function >=-
    function SliceSlider (hObj,event, Img)
        ...
        if exist('Positions.mat','file')~=0
            aux=load('Positions.mat');
        ...
        if length(aux)>1   
            ...
            if exist('positions','var')~=0
                positions2=cellfun(@(x) GetPositions(x,S), positions,'UniformOutput',false);
                for J=1:length(positions2)
                    hold on, plot(positions2{J}(:,1),positions2{J}(:,2),'Color',ColorK)
                end
            end
        else
