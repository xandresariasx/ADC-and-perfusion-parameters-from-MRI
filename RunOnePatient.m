...
Options.GlobalRegisterDWI=1;           % Option to register the images per visit.
Options.AnnotateDWI=1;                   % Option to manually annotate tissues in DW-MRI images. Name labels as: Tumor1, Tumor2,...                                            
Options.LocalRegisterDWI=1;             % Option to do local registration around tumors. If local registration is not needed the options below should be 0.  
    Options.roiFactorDW=0;              % Factor to resize tumor roi (0: no local registration).
Options.GetParDWI=1;                       % Option to get ADC maps                  
...
Options.AnnotateT1W=1;      % Option to manually annotate tissues in T1-MRI images.
Options.GlobalRegisterT1W=1;    % Option to register the images per visit.
...
Options.LocalRegisterT1W=1;      % Option to do local registration around tumors and artery. If local registration is not needed the options below should be 0.  
    Options.roiFactorT1=[0];       % ROI factor per tumor: ROI size is equal to factor multiplied by bounding box around tumors.            
    Options.IncludeArteryLR=[0];   % 1 if the artery should be included in the local registration
Options.GetT1=1;                     % Option to compute T1 maps
    ...
    Options.CorrectFA=0;         % Correct FA per image in case of big intensity bias (requires curve fitting toolbox).
Options.GetConc=1;               % Option to compute contrast curves
    Options.ContrastBrand='Magnevist';        % Multihance, Magnevist
    Options.ScaleMo=0;                                 % If different image scaling between DCE and T1-MRI.
...
Options.GetPar=1;                 % Option to compute iAUC, Ktrans, Vp, Ve parameter maps.
Options.GlobalDateRegisterT1=1;         % Register maps across dates globally.
Options.LocalDateRegisterT1=1;           % Register maps acress dates locally.
    Options.roiFactorLT1=0;                    % ROI factor: ROI is equal to factor multiplied by bounding box around tumors.            
