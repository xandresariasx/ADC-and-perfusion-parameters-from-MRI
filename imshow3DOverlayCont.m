function  imshow3DOverlayCont( Img, disprange,Title,FolderMasks,S,Labels,PositionsPerLabel,MaksPerLabel)
...
if exist([FolderMasks filesep 'Masks.mat'],'file')
    ...
    for I=1:length(Labels)
        ...
        for II=1:length(Positions)
            if Positions{II}(1,3)==S
                hold on, p(I)=plot(Positions{II}(:,1),Positions{II}(:,2),'Color',Colors{I});            
            end
        end    
    end
    if exist('p')
        aux=find(arrayfun(@(x) contains(class(x),'Line'),p));
        legend(p(aux),Labels(aux),'location','northeastoutside')
    end
end
...
txtTitle_Pos = [50 85 uint16(FigPos(3)-100)+1 15];
if exist('Title','var') 
    if ~isempty(Title)
        txtTitle = uicontrol('Style', 'text','Position', txtTitle_Pos,'String',Title{S}, 'BackgroundColor', [0.8 0.8 0.8], 'FontSize', SFntSz);
    end
end
...
if sno > 1
    shand = uicontrol('Style', 'slider','Min',1,'Max',sno,'Value',S,'SliderStep',[1/(sno-1) 10/(sno-1)],'Position', S_Pos,'Callback', {@SliceSlider, Img, FolderMasks});
...
set (gcf, 'WindowScrollWheelFcn', {@mouseScroll, FolderMasks}  );  
...
% -=< Figure resize callback function >=-
    function figureResized(object, eventdata)
        ...
        try,set(txtTitle,'Position', txtTitle_Pos);end
...
% -=< Slice slider callback function >=-
    function SliceSlider (hObj,event, Img, FolderMasks)
...
% -=< Mouse scroll wheel callback function >=-
    function mouseScroll (object, eventdata,FolderMasks)
